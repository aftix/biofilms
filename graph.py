...
def plot_avgstrain(avgx: List[float], avgy: List[float], avgdisp: List[float], times: List[float], name: str='avgstrain_') -> None:
    fig, ax = plt.subplots()
    ax.plot(times, avgx, color='k', marker='.', label='Average $x$ displacement', linestyle='None')
    ax.set_xlabel('Time')
    ax.set_ylabel('Average $x$ displacement')
    fig.savefig(name + 'xoff.png')
    plt.close(fig)
    fig, ax = plt.subplots()
    ax.plot(times, avgy, color='k', marker='.', label='Average $y$ displacement', linestyle='None')
    ax.set_xlabel('Time')
    ax.set_ylabel('Average $y$ displacement')
    fig.savefig(name + 'yoff.png')
    plt.close(fig)
    fig, ax = plt.subplots()
    ax.plot(times, avgdisp, color='k', marker='.', label='Average distance', linestyle='None')
    ax.set_xlabel('Time')
    ax.set_ylabel('Average distance')
    fig.savefig(name + 'dist.png')
    plt.close(fig)
